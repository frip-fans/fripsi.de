...
async function init():Promise<void>{
  ...
  let stops=payload.stops,index=stops.length-1,timer:ReturnType<typeof setTimeout>|undefined,animationFrame:number|undefined,playing=false,runId=0,sliderStartPending=false;
  ...
  range.addEventListener("input",()=>{stopPlay();sliderStartPending=true;render(nearest(+range.value));});range.addEventListener("change",()=>render(nearest(+range.value),true));filter.addEventListener("change",applyFilter);window.addEventListener("resize",buildTicks);
  ...
  const startPlay=(keepPosition=false)=>{if(!keepPosition&&!sliderStartPending)render(0,true);sliderStartPending=false;playing=true;const token=++runId;play.dataset.playing="true";play.setAttribute("aria-pressed","true");playText.textContent=container.dataset.pauseLabel??"Pause";playSoundtrack();const advance=()=>{if(!playing||token!==runId)return;if(index>=stops.length-1){stopPlay();return;}animateTo(index+1,token,()=>{timer=setTimeout(()=>retractLatest(token,()=>{timer=setTimeout(advance,Math.round(500*speedScale()));}),Math.round(400*speedScale()));});};advance();};
  play.addEventListener("click",()=>{if(playing){stopPlay();return;}startPlay();});
  speed.addEventListener("change",()=>{if(playing){stopPlay();startPlay(true);}});document.addEventListener("visibilitychange",()=>{if(document.hidden)stopPlay();});
