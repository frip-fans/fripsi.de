...
3. **管理后台属于网站本身。** `/admin` 提供活动管理、草稿、预览、发布、导入导出和审计记录。
4. **MCP 单独部署为 Remote MCP Worker。** 它与网站共用业务规则和 D1，但不与公开路由混在同一个运行入口。
5. **AI 采用“提案优先”模型。** AI 先创建 `change_set`，预览后才允许发布；不直接修改数据库行。
6. **所有写操作都可追踪。** 保存操作者、来源、变更前后数据、请求 ID 和时间。
7. **不向 AI 暴露 SQL、通用 HTTP 请求或硬删除工具。** 删除统一转为可恢复的归档。
8. **身份认证交给 Cloudflare Access/OAuth。** 不自行实现密码、密码找回或会话系统。
9. **内容更新不触发重新构建。** 发布后写入 D1，前台通过 SSR/API 读取最新数据。
10. **数据必须可迁移。** 使用 SQLite 兼容的简单表结构、仓库内 SQL migration，并提供 CSV 导入导出。
